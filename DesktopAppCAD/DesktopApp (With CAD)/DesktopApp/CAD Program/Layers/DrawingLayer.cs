...
{
    public class DrawingLayer : ICanvasLayer, Utils.ISerialize
    {
        string m_id;
        string m_name = "<Layer>";
        Color m_color;
        ...
        public void GetObjectData(XmlWriter wr)
        {
            ...
            {
                if (drawobj is Utils.ISerialize)
                    ((Utils.ISerialize)drawobj).GetObjectData(wr);
            }
            wr.WriteEndElement();
            wr.WriteEndElement();
        }
        ...
        public static DrawingLayer NewLayer(XmlElement xmlelement)
        {
            ...
            {
                Utils.XmlUtil.ParseProperty(node, layer);
                if (node.Name == "items")
                {
                    foreach (XmlElement itemnode in node.ChildNodes)
                    {
                        ...
                        if (item == null)
                            continue;
                        if (item != null)
                            Utils.XmlUtil.ParseProperties(itemnode, item);
                        if (item is Utils.ISerialize)
                            ((Utils.ISerialize)item).AfterSerializedIn();
