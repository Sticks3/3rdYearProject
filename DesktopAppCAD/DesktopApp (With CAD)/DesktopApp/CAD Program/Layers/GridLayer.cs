...
namespace DesktopApp.CAD_Program.Layers
{
    public class GridLayer : ICanvasLayer, Utils.ISerialize
    {
        public enum eStyle
        {
            Dots,
        ...
        public void Draw(ICanvas canvas, RectangleF unitrect)
        {
            ...
            PointF rightpoint = Utils.ScreenUtils.RightPoint(canvas, unitrect);
            ...
            if (GridStyle == eStyle.Lines)
            {
                ...
                while (left < right)
                {
                    PointF p1 = canvas.ToScreen(new Utils.UnitPoint(left, leftpoint.Y));
                    PointF p2 = canvas.ToScreen(new Utils.UnitPoint(left, rightpoint.Y));
                    path.AddLine(p1, p2);
                    path.CloseFigure();
                    left += gridX;
                }
                ...
                while (bottom < top)
                {
                    PointF p1 = canvas.ToScreen(new Utils.UnitPoint(leftpoint.X, bottom));
                    PointF p2 = canvas.ToScreen(new Utils.UnitPoint(rightpoint.X, bottom));
                    path.AddLine(p1, p2);
                    path.CloseFigure();
                    bottom += gridY;
                }
        ...
        public ISnapPoint SnapPoint(ICanvas canvas, Utils.UnitPoint point, List<IDrawObject> otherobj)
        {
            if (Enabled == false)
                return null;
            Utils.UnitPoint snappoint = new Utils.UnitPoint();
