...
namespace DesktopApp.CAD_Program
{
    public class DataModel : IModel
    {
        ...
        Utils.UndoRedoBuffer m_undoBuffer = new Utils.UndoRedoBuffer();

        Utils.UnitPoint m_centerPoint = Utils.UnitPoint.Empty;
        [Utils.XmlSerializable]
        public Utils.UnitPoint CenterPoint
        {
        ...
        float m_zoom = 1;
        Layers.GridLayer m_gridLayer = new Layers.GridLayer();
        Layers.BackgroundLayer m_backgroundLayer = new Layers.BackgroundLayer();
        ...
        public DataModel()
        {
            ...
            m_toolTypes[DrawTools.LineFuse.ObjectType] = typeof(DrawTools.LineFuse);
            DefaultLayer();
            m_centerPoint = new Utils.UnitPoint(0, 0);
        }
        ...
        public void Save(string filename)
        {
            try
            {
                ...
                {
                    if (layer is Utils.ISerialize)
                        ((Utils.ISerialize)layer).GetObjectData(wr);
                }
        ...
        public bool Load(string filename)
        {
            try
            {
                ...
                foreach (XmlElement childnode in root.ChildNodes)
                {
                    ...
                    {
                        Utils.XmlUtil.ParseProperties(childnode, m_backgroundLayer);
                        continue;
                    }
                    if (childnode.Name == "gridlayer")
                    {
                        Utils.XmlUtil.ParseProperties(childnode, m_gridLayer);
                        continue;
                    }
                    if (childnode.Name == "layer")
                    {
                        Layers.DrawingLayer l = CAD_Program.Layers.DrawingLayer.NewLayer(childnode as XmlElement);
                        m_layers.Add(l);
                    }
                    if (childnode.Name == "property")
                        Utils.XmlUtil.ParseProperty(childnode, this);
                }
                return true;
            }
            catch (Exception e)
        ...
        {
            m_layers.Clear();
            m_layers.Add(new Layers.DrawingLayer("layer0", "Hairline Layer", Color.White, 0.0f));
            m_layers.Add(new Layers.DrawingLayer("layer1", "0.005 Layer", Color.Red, 0.005f));
            m_layers.Add(new Layers.DrawingLayer("layer2", "0.025 Layer", Color.Green, 0.025f));
        }
        ...
        public IDrawObject CreateObject(string type, Utils.UnitPoint point, ISnapPoint snappoint)
        {
            Layers.DrawingLayer layer = ActiveLayer as Layers.DrawingLayer;
        ...
        public void AddObject(ICanvasLayer layer, IDrawObject drawobject)
        {
            ...
            if (m_undoBuffer.CanCapture)
                m_undoBuffer.AddCommand(new Utils.EditCommandAdd(layer, drawobject));
            ((Layers.DrawingLayer)layer).AddObject(drawobject);
        }
        public void DeleteObjects(IEnumerable<IDrawObject> objects)
        {
            Utils.EditCommandRemove undocommand = null;
            if (m_undoBuffer.CanCapture)
                undocommand = new Utils.EditCommandRemove();
            foreach (ICanvasLayer layer in m_layers)
            {
                List<IDrawObject> removedobjects = ((Layers.DrawingLayer)layer).DeleteObjects(objects);
        ...
        public void MoveObjects(Utils.UnitPoint offset, IEnumerable<IDrawObject> objects)
        {
            if (m_undoBuffer.CanCapture)
                m_undoBuffer.AddCommand(new Utils.EditCommandMove(offset, objects));
            foreach (IDrawObject obj in objects)
                obj.Move(offset);
        }
        public void CopyObjects(Utils.UnitPoint offset, IEnumerable<IDrawObject> objects)
        {
            ...
            foreach (IDrawObject obj in objects)
            {
                ...
                newobj.Move(offset);
                ((Layers.DrawingLayer)ActiveLayer).AddObject(newobj);
                AddSelectedObject(newobj);
            }
            if (m_undoBuffer.CanCapture)
                m_undoBuffer.AddCommand(new Utils.EditCommandAdd(ActiveLayer, newobjects));
        }
        public void AfterEditObjects(IEditTool edittool)
        {
            edittool.Finished();
            if (m_undoBuffer.CanCapture)
                m_undoBuffer.AddCommand(new Utils.EditCommandEditTool(edittool));
        }
        ...
        public void MoveNodes(Utils.UnitPoint position, IEnumerable<INodePoint> nodes)
        {
            if (m_undoBuffer.CanCapture)
                m_undoBuffer.AddCommand(new Utils.EditCommandNodeMove(nodes));
        ...
        public List<IDrawObject> GetHitObjects(ICanvas canvas, Utils.UnitPoint point)
        {
            List<IDrawObject> selected = new List<IDrawObject>();
            foreach (ICanvasLayer layer in m_layers)
            {
        ...
        public ISnapPoint SnapPoint(ICanvas canvas, Utils.UnitPoint point, Type[] runningsnaptypes, Type usersnaptype)
        {
