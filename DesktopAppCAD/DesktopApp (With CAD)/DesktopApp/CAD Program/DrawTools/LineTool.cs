...
namespace DesktopApp.CAD_Program.DrawTools
{
    class NodePointLine : INodePoint
    {
        ...
        public void SetPosition(Utils.UnitPoint pos)
        {
            ...
            if (m_angleLocked || Control.ModifierKeys == (Keys)(Keys.Control | Keys.Shift))
                pos = Utils.HitUtil.NearestPointOnLine(m_owner.P1, m_owner.P2, pos, true);
        ...
        #endregion
        protected Utils.UnitPoint GetPoint(ePoint pointid)
        {
        ...
        protected Utils.UnitPoint OtherPoint(ePoint currentpointid)
        {
            if (currentpointid == ePoint.P1)
                return GetPoint(ePoint.P2);
            return GetPoint(ePoint.P1);
        }
        protected void SetPoint(ePoint pointid, Utils.UnitPoint point, Line line)
        {
            if (pointid == ePoint.P1)
                line.P1 = point;
            if (pointid == ePoint.P2)
    ...
    class Line : DrawObjectBase, IDrawObject, Utils.ISerialize
    {
        protected Utils.UnitPoint m_p1, m_p2;
        ...
        public Line(Utils.UnitPoint point, Utils.UnitPoint endpoint, float width, Color color)
        {
            P1 = point;
            P2 = endpoint;
            Width = width;
        ...
        public RectangleF GetBoundingRect(ICanvas canvas)
        {
            ...
            return Utils.ScreenUtils.GetRect(m_p1, m_p2, thWidth);
        }
        ...
        {
            if (Control.ModifierKeys == Keys.Control)
                point = Utils.HitUtil.OrthoPointD(m_p1, point, 45);
            m_p2 = point;
        }
        public virtual eDrawObjectMouseDown OnMouseDown(ICanvas canvas, Utils.UnitPoint point, ISnapPoint snappoint)
        {
            ...
            if (snappoint is PerpendicularSnapPoint && snappoint.Owner is Line)
            {
                ...
                m_p2 = Utils.HitUtil.NearestPointOnLine(src.P1, src.P2, m_p1, true);
            ...
            if (snappoint is PerpendicularSnapPoint && snappoint.Owner is Arc)
            {
                ...
                m_p2 = Utils.HitUtil.NearestPointOnCircle(src.Center, src.Radius, m_p1, 0);
                return eDrawObjectMouseDown.DoneRepeat;
            }
            if (Control.ModifierKeys == Keys.Control)
                point = Utils.HitUtil.OrthoPointD(m_p1, point, 45);
        ...
        {
            float thWidth = ThresholdWidth(canvas, Width);
            if (Utils.HitUtil.CircleHitPoint(m_p1, thWidth, point))
                return new NodePointLine(this, NodePointLine.ePoint.P1);
            if (Utils.HitUtil.CircleHitPoint(m_p2, thWidth, point))
        ...
        public ISnapPoint SnapPoint(ICanvas canvas, Utils.UnitPoint point, List<IDrawObject> otherobjs, Type[] runningsnaptypes, Type usersnaptype)
        {
            ...
            if (runningsnaptypes != null)
            {
                foreach (Type snaptype in runningsnaptypes)
                {
                    ...
                    {
                        Utils.UnitPoint p = MidPoint(canvas, m_p1, m_p2, point);
                        if (p != Utils.UnitPoint.Empty)
                            return new MidpointSnapPoint(canvas, this, p);
                    }
                    if (snaptype == typeof(IntersectSnapPoint))
                    {
                        Line otherline = Utils.Utils.FindObjectTypeInList(this, otherobjs, typeof(Line)) as Line;
                        if (otherline == null)
                            continue;
                        Utils.UnitPoint p = Utils.HitUtil.LinesIntersectPoint(m_p1, m_p2, otherline.m_p1, otherline.m_p2);
                        if (p != Utils.UnitPoint.Empty)
            ...
            if (usersnaptype == typeof(MidpointSnapPoint))
                return new MidpointSnapPoint(canvas, this, Utils.HitUtil.LineMidpoint(m_p1, m_p2));
            ...
            {
                double d1 = Utils.HitUtil.Distance(point, m_p1);
                double d2 = Utils.HitUtil.Distance(point, m_p2);
                if (d1 <= d2)
                    return new VertextSnapPoint(canvas, this, m_p1);
                return new VertextSnapPoint(canvas, this, m_p2);
            }
            ...
            {
                Utils.UnitPoint p = Utils.HitUtil.NearestPointOnLine(m_p1, m_p2, point);
                if (p != Utils.UnitPoint.Empty)
        ...
        public string GetInfoAsString()
        {
            return string.Format("Line@{0},{1} - L={2:f4}<{3:f4}",
                P1.PosAsString(),
                P2.PosAsString(),
                Utils.HitUtil.Distance(P1, P2),
                Utils.HitUtil.RadiansToDegrees(Utils.HitUtil.LineAngleR(P1, P2, 0)));
        }
        #endregion
        #region ISerialize
        ...
        {
            Utils.UnitPoint newlinepoint = Utils.HitUtil.NearestPointOnLine(P1, P2, newpoint, true);
            if (Utils.HitUtil.Distance(newlinepoint, P1) < Utils.HitUtil.Distance(newlinepoint, P2))
                P1 = newlinepoint;
            else
                P2 = newlinepoint;
        }
    ...
    class LineEdit : Line, IObjectEditInstance
    {
        ...
        protected virtual void MouseMovePerpendicular(ICanvas canvas, Utils.UnitPoint point)
        {
            if (m_perSnap.Owner is Line)
            {
                ...
                m_p1 = Utils.HitUtil.NearestPointOnLine(src.P1, src.P2, point, true);
                m_p2 = point;
            }
            if (m_perSnap.Owner is IArc)
            {
                IArc src = m_perSnap.Owner as IArc;
                m_p1 = Utils.HitUtil.NearestPointOnCircle(src.Center, src.Radius, point, 0);
                m_p2 = point;
            }
        }
        protected virtual void MouseMoveTangent(ICanvas canvas, Utils.UnitPoint point)
        {
            if (m_tanSnap.Owner is IArc)
            {
                ...
                m_p1 = Utils.HitUtil.TangentPointOnCircle(src.Center, src.Radius, point, m_tanReverse);
                m_p2 = point;
                if (m_p1 == Utils.UnitPoint.Empty)
