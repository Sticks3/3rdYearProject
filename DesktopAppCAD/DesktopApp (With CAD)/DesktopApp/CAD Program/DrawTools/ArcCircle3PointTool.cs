...
namespace DesktopApp.CAD_Program.DrawTools
{
    ...
    {
        protected DrawTools.Arc3Point m_owner;
        protected DrawTools.Arc3Point m_clone;
        protected Utils.UnitPoint[] m_originalPoints = new Utils.UnitPoint[3];
        protected Utils.UnitPoint[] m_endPoints = new Utils.UnitPoint[3];
         ...
         * */
        void SetPoint(Arc3Point arc, Utils.UnitPoint pos)
        {
            if (m_curPoint == Arc3Point.eCurrentPoint.p1)
                arc.P1 = pos;
            if (m_curPoint == Arc3Point.eCurrentPoint.p2)
            ...
            double angleR = Utils.HitUtil.LineAngleR(arc.Center, pos, angleToRound);

            if (m_curPoint == Arc3Point.eCurrentPoint.startangle)
                arc.P1 = Utils.HitUtil.PointOncircle(arc.Center, arc.Radius, angleR);
            if (m_curPoint == Arc3Point.eCurrentPoint.endangle)
                arc.P3 = Utils.HitUtil.PointOncircle(arc.Center, arc.Radius, angleR);
            if (m_curPoint == Arc3Point.eCurrentPoint.radius)
            {
                double radius = Utils.HitUtil.Distance(arc.Center, pos);
                arc.P1 = Utils.HitUtil.PointOncircle(arc.Center, radius, Utils.HitUtil.DegressToRadians(arc.StartAngle));
                arc.P2 = pos;
                arc.P3 = Utils.HitUtil.PointOncircle(arc.Center, radius, Utils.HitUtil.DegressToRadians(arc.EndAngle));
            }
    ...
    class Arc3Point : DrawObjectBase, IArc, IDrawObject, Utils.ISerialize
    {
        ...
        Utils.UnitPoint m_p1 = Utils.UnitPoint.Empty;
        Utils.UnitPoint m_p2 = Utils.UnitPoint.Empty;
        Utils.UnitPoint m_p3 = Utils.UnitPoint.Empty;
        [Utils.XmlSerializable]
        public Utils.UnitPoint P1
        {
            get { return m_p1; }
            set { m_p1 = value; }
        }
        [Utils.XmlSerializable]
        public Utils.UnitPoint P2
        {
            get { return m_p2; }
            set { m_p2 = value; }
        }
        [Utils.XmlSerializable]
        public Utils.UnitPoint P3
        {
            get { return m_p3; }
            set { m_p3 = value; }
        }
        ...
        public RectangleF GetBoundingRect(ICanvas canvas)
        {
            ...
            if (m_p2.IsEmpty || m_p3.IsEmpty)
                rect = Utils.ScreenUtils.GetRect(m_p1, m_lastPoint, thWidth);

            if (rect.IsEmpty)
            {
                float r = m_radius + thWidth / 2;
                rect = Utils.HitUtil.CircleBoundingRect(m_center, r);
                if (Selected)
                {
        ...
        void DrawArc(ICanvas canvas, RectangleF unitrect)
        {
            ...
            if (Selected)
            {
                ...
                if (m_p1.IsEmpty == false)
                {
                    ...
                    Utils.UnitPoint anglepoint = StartAngleNodePoint(canvas);
        ...
        bool PointsInLine()
        {
            double slope1 = Utils.HitUtil.LineSlope(P1, P2);
            double slope2 = Utils.HitUtil.LineSlope(P1, P3);
        ...
        public void UpdateCenterNodeFromAngles()
        {
            ...
            P2 = Utils.HitUtil.PointOncircle(m_center, m_radius, Utils.HitUtil.DegressToRadians(angle));
        }
        public void UpdateArcFrom3Points()
        {
            m_center = Utils.HitUtil.CenterPointFrom3Points(m_p1, m_p2, m_p3);
            m_radius = (float)Utils.HitUtil.Distance(m_center, m_p1);
            StartAngle = (float)Utils.HitUtil.RadiansToDegrees(Utils.HitUtil.LineAngleR(m_center, m_p1, 0));
            EndAngle = (float)Utils.HitUtil.RadiansToDegrees(Utils.HitUtil.LineAngleR(m_center, m_p3, 0));
            // find angle from P1 on line P1|P3 to P2. If this angle is 0-180 the direction is CCW else it is CW

            double p1p3angle = Utils.HitUtil.RadiansToDegrees(Utils.HitUtil.LineAngleR(m_p1, m_p3, 0));
            double p1p2angle = Utils.HitUtil.RadiansToDegrees(Utils.HitUtil.LineAngleR(m_p1, m_p2, 0));
        ...
        void MoveMouse3P132(ICanvas canvas, Utils.UnitPoint point)
        {
        ...
        void MoveMouse3P123(ICanvas canvas, Utils.UnitPoint point)
        {
            if (m_curPoint == eCurrentPoint.p1)
            {
                m_p1 = point;
        ...
        eDrawObjectMouseDown MouseDown3P132(ICanvas canvas, Utils.UnitPoint point, ISnapPoint snappoint)
        {
        ...
        eDrawObjectMouseDown MouseDown3P123(ICanvas canvas, Utils.UnitPoint point, ISnapPoint snappoint)
        {
            OnMouseMove(canvas, point);
            if (m_curPoint == eCurrentPoint.p1)
            {
        ...
        Utils.UnitPoint StartAngleNodePoint(ICanvas canvas)
        {
            double r = Radius + canvas.ToUnit(8);
            return Utils.HitUtil.PointOncircle(m_center, r, Utils.HitUtil.DegressToRadians(StartAngle));
        }
        Utils.UnitPoint EndAngleNodePoint(ICanvas canvas)
        {
            double r = Radius + canvas.ToUnit(8);
            return Utils.HitUtil.PointOncircle(m_center, r, Utils.HitUtil.DegressToRadians(EndAngle));
        }
        Utils.UnitPoint RadiusNodePoint(ICanvas canvas)
        {
            double r = Radius + canvas.ToUnit(8);
            float angle = StartAngle + SweepAngle / 2;
            return Utils.HitUtil.PointOncircle(m_center, r, Utils.HitUtil.DegressToRadians(angle));
        }
        ...
        {
            float thWidth = Line.ThresholdWidth(canvas, Width, ThresholdPixel);
            if (Utils.HitUtil.PointInPoint(P1, point, thWidth))
            {
                m_lastPoint = P1;
                return new NodePointArc3PointPoint(this, eCurrentPoint.p1);
            }
            if (Utils.HitUtil.PointInPoint(P2, point, thWidth))
            {
                m_lastPoint = P2;
                return new NodePointArc3PointPoint(this, eCurrentPoint.p2);
            }
            if (Utils.HitUtil.PointInPoint(P3, point, thWidth))
            {
                m_lastPoint = P3;
                return new NodePointArc3PointPoint(this, eCurrentPoint.p3);
            }
            Utils.UnitPoint p = StartAngleNodePoint(canvas);
            if (Utils.HitUtil.PointInPoint(p, point, thWidth))
            {
                m_lastPoint = p;
        ...
        protected Utils.UnitPoint m_lastPoint = Utils.UnitPoint.Empty;
    }
}
